...
export const createBookCategory = async (bookId: number, categoryId: number) => {
    try {
        ...
        const existingQuery = 'SELECT * FROM "Book_Category" WHERE book_id = $1 AND category_id = $2';
        ...
        // Créer la relation
        const insertQuery = 'INSERT INTO "Book_Category" (book_id, category_id) VALUES ($1, $2) RETURNING *';
...
export const updateBookCategory = async (oldBookId: number, oldCategoryId: number, newBookId: number, newCategoryId: number) => {
    try {
        ...
        const newExistingQuery = 'SELECT * FROM "Book_Category" WHERE book_id = $1 AND category_id = $2';
...
export const deleteBookCategory = async (bookId: number, categoryId: number) => {
    try {
        ...
        const deleteQuery = 'DELETE FROM "Book_Category" WHERE book_id = $1 AND category_id = $2 RETURNING *';
